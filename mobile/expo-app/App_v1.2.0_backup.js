 ...
 * 
 * Version 1.1.3
 * WebView-basierte App mit nativem Audio-Metering
 * Das Mikrofon wird nativ aufgenommen und die Pegel an die WebView gesendet
 */
...
export default function App() {
  ...
  // ============================================================================
  // Native Audio Recording für Pegel-Anzeige
  // ============================================================================
  
  const startNativeAudio = async () => {
    try {
      console.log('Starting native audio metering...');
      ...
      const { recording } = await Audio.Recording.createAsync(
        {
          ...
          web: {},
          isMeteringEnabled: true, // Wichtig für Pegel!
        },
        (status) => {
          // Callback für Metering-Updates
          if (status.isRecording && status.metering !== undefined) {
            // Metering ist in dB, typisch -160 bis 0
            // Konvertiere zu 0-100%
            const db = status.metering;
            const normalizedLevel = Math.max(0, Math.min(100, (db + 60) * 1.67));
            setAudioLevel(normalizedLevel);
            
            // Sende Pegel an WebView
            if (webViewRef.current) {
              webViewRef.current.injectJavaScript(`
                if (window.updateNativeAudioLevel) {
                  window.updateNativeAudioLevel(${normalizedLevel}, ${db});
                }
                true;
              `);
            }
      ...
      setIsRecording(true);
      console.log('Native audio metering started');
  ...
  const requestPermissions = async () => {
    try {
      ...
      if (audioStatus === 'granted') {
        // Starte native Audio-Aufnahme für Pegel
        setTimeout(() => startNativeAudio(), 1000);
      }
  ...
  // JavaScript zum Injizieren in WebView (für native Audio-Integration)
    ...
    (function() {
      ...
      window.updateNativeAudioLevel = function(level, db) {
        ...
        if (levelFill) {
          ...
          // Farbe je nach Pegel
          if (level > 80) {
            levelFill.style.background = 'linear-gradient(90deg, #4ECDC4, #ff6b6b)';
          } else if (level > 50) {
            levelFill.style.background = 'linear-gradient(90deg, #4ECDC4, #ffd93d)';
          } else {
            levelFill.style.background = 'linear-gradient(90deg, #4ECDC4, #51cf66)';
          }
        }
        if (levelDb) {
        ...
        // Einfache Waveform-Animation basierend auf Pegel
        const waveCanvas = document.getElementById('waveformCanvas');
        if (waveCanvas && window.waveformCtx) {
          const ctx = window.waveformCtx;
          ...
          // Draw waveform based on level
          ctx.strokeStyle = '#4ECDC4';
          ctx.lineWidth = 2;
          ctx.beginPath();
          
          const amplitude = (level / 100) * (height / 2 - 10);
          const time = Date.now() / 100;
          
          for (let x = 0; x < width; x++) {
            const y = height / 2 + Math.sin(x * 0.03 + time) * amplitude * Math.sin(x * 0.01);
        ...
        if (spectroCanvas && window.spectrogramCtx) {
          const ctx = window.spectrogramCtx;
          ...
          // Draw new column based on level
          for (let y = 0; y < height; y++) {
            const freq = 1 - (y / height);
            const intensity = (level / 100) * (0.3 + 0.7 * Math.random()) * 
                             (freq > 0.2 && freq < 0.8 ? 1 : 0.3);
            const hue = 240 - intensity * 180;
            ctx.fillStyle = 'hsla(' + hue + ', 80%, ' + (20 + intensity * 40) + '%, 1)';
            ctx.fillRect(width - 1, y, 1, 1);
          }
        }
      };
      
      // Canvas-Kontexte für native Updates speichern
      setTimeout(function() {
        const spectroCanvas = document.getElementById('spectrogramCanvas');
        const waveCanvas = document.getElementById('waveformCanvas');
        if (spectroCanvas) window.spectrogramCtx = spectroCanvas.getContext('2d');
        if (waveCanvas) window.waveformCtx = waveCanvas.getContext('2d');
        console.log('Native audio integration ready');
      }, 1000);
      
      console.log('BirdSound App v${APP_VERSION} - Native Mode');
      true;
    })();
  `;
